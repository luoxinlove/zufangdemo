...
<META name=GENERATOR >
    ...
    <script language="JavaScript">
            //点击事件
            function chen(obj,id) {
                //发送异步请求
                $.post("delHouse", {"id": id}, function (date) {
                    if (date.result > 0) {
...
<DIV class=search>欢迎${sessionScope.userinfo.name}
...
<LABEL class=ui-green><INPUT onclick='document.location="index.jsp"' value="退       出" type=button name=search></LABEL>
    <INPUT onclick="location.href='searchHouse?id=${h.id}'" value="修    改" type=button name=search>
</DIV></DIV>
<DIV class="main wrap">
<DIV id=houseArea>
        ...
        <img src="http:/localhost:80/${h.path}" width="100" height="75" alt=""></A></SPAN></TD>
          <TD>
              <DL>
                  ...
                  <DD>${h.dname}=${h.sname},类型:${h.tname}=面积:${h.floorage}平米<BR>联系方式：${h.contact}
                  </DD>
              </DL>
          </TD>
          <TD class=house-type>
          ...
          <TD class=house-price><LABEL class=ui-green><INPUT value="删    除" id="delHouse" type=button name=sss onclick="chen(this,${h.id})"></LABEL></TD>
      </TR>
  </c:forEach>
...
dojoType="struts:BindAnchor">100</A>
   </LI>
